...
- **封闭的第一方源清单**——未知 v1 事件会使迁移失败，包括带有 `ignorable: true` 的事件，除非安装构建声明了该事件类型；已声明的事件作为不透明记录迁移。
