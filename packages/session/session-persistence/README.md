...
A stored log the current build cannot faithfully interpret is refused with a direction-aware error, never misread. `SESSION_FORMAT_VERSION` remains v0 and this build provides no format-migration path; a newer version instructs the operator to upgrade the harness. The decoder accepts only the bounded same-version record variants named below. An event type unknown to this build refuses unless its envelope marks it `ignorable` or its active plugin registered it through `ctx.sessionEventTypes` before the read; committed-prefix corruption rejects as `SessionPersistenceCorruptionError` ([rationale](../../../.agents/notes/implemented/simplification/2026-08-25-fail-closed-session-event-vocabulary.md); [registration seam](../../../.agents/notes/implemented/architecture/2026-08-28-session-event-registration.md); [ignorable compatibility](../../../.agents/notes/implemented/architecture/2026-08-30-retain-ignorable-external-session-events.md)).
