...
A stored log the current build cannot faithfully interpret is refused with a direction-aware error, never misread. `SessionHandle` exposes only current logical v1 records; a provider must convert any supported historical storage before returning a handle, and the shipped JSONL provider migrates released v0 through its static catalog. A newer format instructs the operator to upgrade the harness. An event type unknown to this build refuses unless its envelope marks it `ignorable` or its active plugin registered it through `ctx.sessionEventTypes` before the read, and committed-prefix corruption rejects as `SessionPersistenceCorruptionError` ([registration seam](../../../.agents/notes/implemented/architecture/2026-08-28-session-event-registration.md)).
