...
当前构建无法忠实解读的存储日志会以方向感知的错误被拒绝，绝不错读。`SessionHandle` 只暴露当前逻辑 v1 记录；provider 必须在返回句柄前转换任何受支持的历史存储，随产品交付的 JSONL provider 会通过静态 catalog 迁移已发布 v0。更高格式会要求操作者升级 harness。本构建不认识的事件类型会被拒绝，除非其信封标记为 `ignorable`，或其活动插件已在读取前通过 `ctx.sessionEventTypes` 注册；已提交前缀中的损坏以 `SessionPersistenceCorruptionError` 拒绝（[注册 seam](../../../.agents/notes/implemented/architecture/2026-08-28-session-event-registration.zh.md)）。
