...
生成的 `KNOWN_SESSION_EVENT_TYPES` 集合覆盖本仓库中的事件声明。持久化仓库外必需事件的插件，必须在任何历史读取可以解释这些事件之前，通过 `ctx.sessionEventTypes` 注册它们：
...
| [`src/event-types.ts`](src/event-types.ts) | 外部必需事件类型的 effect 所有制接纳注册表 |
...
- **`SESSION_FORMAT_VERSION` 只命名当前逻辑表示**——历史 header 与事件位于相邻格式包中，持久化会在构造 `Session` 前发布完整受支持链；同版本未知事件仍要求信封显式带有 `ignorable` 标记，或由其活动插件通过 `ctx.sessionEventTypes` 注册（[机制](../../../.agents/notes/implemented/architecture/2026-08-31-released-session-format-migrations.zh.md)；[外部注册](../../../.agents/notes/implemented/architecture/2026-08-28-session-event-registration.zh.md)）。
