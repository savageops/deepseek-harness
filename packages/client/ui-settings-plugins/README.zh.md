...
`dsh-client-ui-settings-plugins` 是 dsh Web 客户端的**插件**设置分区：用户在其**插件配置**标签页上编辑宿主平面插件配置，功能插件则通过 `settings.plugins.tab` 贡献自己的页面。本包自己的标签页为每个配置由用户拥有的 Host 插件展示一张可展开卡片：卡片展示插件名称及其管辖范围，展开后是绑定到该插件 settings 命名空间的手写控件，每个字段标注用户是否覆盖过它，并提供重置回部署组装值的入口。卡片暂存用户输入，只有用户保存时才写入，且每次写入都以表单读取时的命名空间 revision 设栅。Subagent 卡还可以选择接收后续委派的已注册子运行时。
...
Subagent 卡会同时暂存其子运行时、开关、自动默认路由与精确的按次模型复选框。启用时必须设置默认 provider/model 路由，或至少选择一条适配器路由。默认路由使用所选模型的实时推理强度选项；允许列表仍是模型按次覆盖路由时的独立授权。保存会在一次 mutation 中提交 `runtimeProvider`、`enabled`、存在时的 `defaultSelection` 与 `allowedModels`，并以草稿开始时的 revision 设栅；Host revision 更新后，草稿会标记为失败，而不会恢复已撤销的路由。关闭时会保留已选运行时、路由与允许列表供以后重新使用。可用模型按提供方分组；当前目录中缺失的已存路由排在末尾，且仍可移除。适配器名称、模型描述、推理强度标签与运行时描述仍属于实时目录元数据，不会存储；适配器变化、设置提交和重连后，卡片会刷新这些元数据。

**子运行时**选择器读取 settings-safe 的 `ctx.subagents.providers` 目录，只保存所选提供方名称。DSH 管理的运行时会显示 DSH 模型与推理强度控件；Codex、Claude Code 或 OpenCode 这类原生运行时会显示所有权提示，并隐藏不兼容的 DSH 控件。浏览器不会收到可执行命令、参数、路径、环境值、凭据或原生模型设置。已保存但当前未注册的提供方仍会以不可用候选显示，因此临时改变 Profile 不会静默改写用户选择。
...
Subagent 卡消费 `ctx.subagents.providers` 这一 Host remote；它只返回注册名称、面向用户的标签、简短描述、产品类型与模型所有权。运行时控制器把这份实时目录与保存的 `runtimeProvider` 合并；缺失的已存名称会保留为不可用候选，并在连接或提供方目录变化后重读。所选名称经由 Host settings scope 写回；可执行配置仍由注册该提供方的 Profile 持有。
...
- **原生运行时保留原生模型控件**：卡片可以选择已注册的 Codex、Claude Code 或 ACP/OpenCode 运行时，但不会代理这些产品的模型目录或推理强度设置；请在选定的子产品中配置这些值。
- **运行时可用性跟随组合**：选择器只列出当前 Host 已注册的提供方。通用 ACP 包需要一个带具体命令与策略的 Profile 行，具名子运行时才会出现。
