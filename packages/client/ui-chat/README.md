...
- [Large history rendering](#large-history-rendering)
...
When the assembled Chat order exceeds 100 nodes, `ChatView` mounts only the measured viewport window with `@tanstack/react-virtual`, keyed by the stable Chat node key and overscanned by 12 rows. The virtual rows are flow roots so per-node spacing remains part of the measured height; Turn navigation scrolls to an index when its target is outside the mounted window. Smaller histories keep the ordinary flow path. All Chat flow items also use `content-visibility: auto` with an intrinsic 160 px fallback, so retained semantic rows do not force full offscreen style, layout, and paint work.
