...
Web 的**插件 → 插件配置 → Subagent** 卡片读取 Host 的 settings-safe 子运行时目录。选择已注册的 `codex` 运行时后，后续委派会路由到本提供方。该选择器选择的是运行时，而不是跨产品的模型覆盖：本提供方声明原生模型所有权，因此 DSH 不会把自己的模型选择字段或推理强度设置应用到 Codex。请在 Codex 本身配置子级模型与推理强度。提供方行的可选 `model` 仍是部署级固定覆盖，会作用于该行创建的每个线程。
...
- **静态实例配置**——Profile 配置项固定提供方名称、可选模型与工具绑定；Web 运行时选择器可以为后续调用选择已注册的提供方实例，但不能更改 Codex 的原生模型或推理强度设置，每个公开工具都需要唯一的 `toolName`。
