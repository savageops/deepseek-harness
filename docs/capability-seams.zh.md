...
flowchart LR
  ...
  svc_sessionEventTypes["ctx.sessionEventTypes<br/>External required event admission"]
  ...
  pkg_session --> svc_sessionEventTypes
  ...
  svc_sessionEventTypes --> pkg_session_persistence
...
| `ctx.sessionEventTypes` | `core` | [`session`](../packages/core/session) | - | [`session-persistence`](../packages/session/session-persistence) | - | 活跃的仓库外事件所有者在此注册必需类型；persistence 仅在所有者可以解释其记录期间接纳这些类型。 |
