...
Every session event type is required-on-read by default. Each backend read validates the log through the persistence seam's shared `validateStoredEvents` storage contract, which compares every event type with `KNOWN_SESSION_EVENT_TYPES`, the generated set of every `SessionEventMap` member declared in this repository. An unknown type refuses reconstruction with `SessionFormatUnsupportedError` unless the record carries the envelope's `ignorable: true` marker or its active external owner registered the type through `ctx.sessionEventTypes`; the diagnostic names the event and sequence, identifies the likely newer writer, and includes the raw artifact path when the backend has one. The guard remains read-side only because rejecting an append after a live event is committed would interrupt durability before the session can report the unsupported log on its next read.

The envelope's optional `ignorable` marker is part of the merged design again: it is the persisted, per-event compatibility classification, and its retention rationale lives in [the retain-ignorable note](../architecture/2026-08-30-retain-ignorable-external-session-events.md). `SESSION_FORMAT_VERSION` remains `0`, and the seam ships no format migration.
...
First-party JSONL session bytes remain unchanged, including packed rows and `SESSION_FORMAT_VERSION = 0`. Existing first-party JSONL sessions remain readable. (The SQLite backend was removed in the handle-based persistence refactor; its schema-18 `is_packed` discriminator described here survives only as history.)

The assembled headless refusal test proves that a user sees the unknown type, sequence, newer-writer direction, and raw JSONL path. Core seed tests reject fields outside the current event envelope; the shared storage-contract suite rejects every unmarked, unregistered unknown type, and the backend contract suites (`runPersistenceContract`, `runLiveWritePathContract`) pin equivalent refusals in every provider. The generated persistence catalog and known-event module keep the reader's set synchronized with repository-owned declarations.
